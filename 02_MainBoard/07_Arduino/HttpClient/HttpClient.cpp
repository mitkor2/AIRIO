...
const char* HttpClient::kUserAgent = "Canary";
...
HttpClient::HttpClient(Client& aClient, const IPAddress& aServerAddress, uint16_t aServerPort)
 : iClient(&aClient), iServerName(""), iServerAddress(aServerAddress), iServerPort(aServerPort),
   iConnectionClose(true), iSendDefaultRequestHeaders(true)
{
  resetState();
}
                                ...
                                const char* aContentType, int aContentLength, const byte aBody[])
{
    ...
    {
        if (iServerName.length() > 0)
        {
            if (!iClient->connect(iServerName.c_str(), iServerPort) > 0)
            {
#ifdef LOGGING
    ...
    if (HTTP_SUCCESS == ret)
    {
        ...
        if (hasBody)
        {
            ...
            while (aContentLength > 0)
            {
                ...
                // TODO - this is modem specific, should be moved to modem driver.
                if (aContentLength > chunk)
                {
                    write(&aBody[offset], chunk);
                    aContentLength -= chunk;
                    offset += chunk;
                }
                else
                {
                    write(&aBody[offset], aContentLength);
                    aContentLength = 0;
                }
            }
        }
...
int HttpClient::sendInitialHeaders(const char* aURLPath, const char* aHttpMethod)
{
    ...
    // project specific
    // sendHeader(HTTP_HEADER_DEVICE_ID, utilsGetUID().c_str());
    // project specific
